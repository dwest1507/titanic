...
## Comparing Models
### AUC
Each model can be compared against each other using the metrice area under the curve (AUC). The AUC for the first random forest model is below:
```{r}
pred <- predict(object = rf_model1,
                newdata = train,
                type = "prob")

auc(actual = train$Survived,
    predicted = pred[,"1"])
```
